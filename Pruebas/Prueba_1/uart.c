...
/**
 * @brief   Envía un carácter por el puerto serial.
 * @param   c  Byte a transmitir.
 */
void UART_SendChar(char c) {
    while (PIR1bits.TXIF == 0);  // Espera a que el buffer TX esté vacío
    TXREG = c;                   // Carga el carácter → dispara la transmisión
}
 ...
 * @details Usa división sucesiva para extraer cada dígito decimal.
 *          Caso especial: si val == 0, envía directamente '0'.
 * @param   val  Número a transmitir (0 – 65535).
 */
void UART_SendUInt(unsigned int val) {
    char buf[6];          // Máx. 5 dígitos + terminador
    unsigned char i = 0;
...
void UART_SendFloat(float val, unsigned char decimals) {
    ...
    /* Calcula multiplicador según decimales pedidos */
    ...
    /* Relleno con ceros a la izquierda si la parte decimal es pequeña */
